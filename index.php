<?php
    ...
    $imgDir = $_SERVER['DOCUMENT_ROOT']. '/uploads/';
    $uploadedImages = array_slice(scandir($imgDir), 2); 

    //converted array to json to read in DOM
    $js_array = json_encode($uploadedImages);
...
//imgs in upload folder converted into JSON
var jsonImgArr = <?php echo $js_array ?>;

//loop through array to create the amount of elements needed for carousel
var htmlElements = "";
for (var i = 0; i < jsonImgArr.length; i++) {
    htmlElements += '<div class="carousel-cell">' + '<img src=./uploads/' + jsonImgArr[i] + ' />' + '</div>';
}
